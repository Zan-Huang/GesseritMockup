...
/**
 * Nudge a point off true. Baking the quiver of a hand into the coordinates
 * keeps it resolution-independent; done with an SVG displacement filter it
 * would force the whole ornament to be rasterised at one fixed size.
 */
const WOBBLE = 0.6;
function wobble([x, y]) {
  return [x + noise2(x, y) * WOBBLE, y + noise2(x + 41.7, y + 137.3) * WOBBLE];
}

function spline(rawPts, closed = true, tension = 1) {
  const pts = rawPts.map(wobble);
  const n = pts.length;
  if (n < 2) return "";
...
function acanthusDef(
  ...
  const clip = def(`<clipPath id="{{id}}"><path d="${silhouette}"/></clipPath>`);
  const parts = [
    `<path d="${silhouette}" fill="url(#${wash})" stroke="${INK}" stroke-width="0.85" ` +
      `stroke-linejoin="round"/>`,
    // pigment pools where the brush stopped, so the ground is darker at the rim
    `<g clip-path="url(#${clip})"><path d="${silhouette}" fill="none" stroke="${p.deep}" ` +
      `stroke-width="3.4" opacity="0.3"/></g>`,
...
/**
 * No filters anywhere: the file is used as a CSS background-image, and a
 * filtered subtree gets rasterised once at a fixed size instead of being
 * redrawn at the device's resolution. The unevenness of tempera comes from the
 * washes and the baked-in wobble instead.
 */
function paintedPage(viewBox, w, h, body) {
  return (
...
function buildVine() {
  ...
  /* Nothing may cross y = 0 or y = TILE_H: the tile repeats down the margin,
     so an ornament that overhangs an end is sliced flat at every seam. */
  const scrolls = [
    { u: 0.24, side: -1, id: leafBlue, s: 0.92 },
  ...
  const r = rng(0x2f19);
  for (const u of [0.1, 0.3, 0.62, 0.8, 1.12, 1.3, 1.6, 1.82, 1.94]) {
    const p = stemAt(u);
    const side = r() > 0.5 ? 1 : -1;
    const [tx, ty] = stemTan(u);
    const off = 14 + r() * 10;
    const x = Math.min(130, Math.max(10, p[0] - ty * off * side));
    const y = Math.min(TILE_H - 6, Math.max(6, p[1] + tx * off * side));
    body.push(at(bezant, x, y, 0, 0.8 + r() * 0.4));
  }

  return paintedPage(`0 0 ${TILE_W} ${TILE_H}`, TILE_W, TILE_H, body);
}
...
function buildTip() {
  ...
  return paintedPage(`0 0 ${TILE_W} 150`, TILE_W, 150, body);
}
...
function buildProof() {
  ...
  return paintedPage(`0 0 ${cols * cell} ${h}`, cols * cell, h, body);
}
